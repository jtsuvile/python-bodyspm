...
def preprocess_subjects(subnums, indataloc, outdataloc, stimuli, bgfiles=None,
                        fieldnames=None,
                        intentionally_empty=False):
    """Reads in data from web interface output and writes the subjects out to
    .csv files (colouring data) and .json (other sub data).
    Also draws single subject's data into file for quality control.
    ...
    :param stimuli : an object of class Stimuli, with information about the
    stimulus file names and whether data has been collected as one- or twosided
    :param bgfiles: list, optional. If you want to read in data from a comma
    separated text file as background information,
    supply a list of files to include.
    :param fieldnames: list of lists, optional.
    List for each background information file giving the names of the
    ...
    for i, subnum in enumerate(subnums):
        print("preprocessing subject " + str(subnum) + " which is " +
              str(i+1) + "/" + str(len(subnums)))
        # make subject
        sub = Subject(subnum)
        sub.read_data(indataloc, stimuli, False, intentionally_empty)
        # if files with background information have been defined,
        # save values to sub.bginfo
...
def add_background_table(new_bgdata, linking_col, subloc,
                         exclude=[], override=True):
    """
    Add new background information from a data frame to a subject after
    preprocessing.
    Adding is done based on a linking column (subject identifier).
    The names of the background variables are taken from columns
    of the data frame.

    :param new_bgdata: table with the new background information
    :param linking_col: column name for the column containing subject
    identifiers
    :param subloc: path to where the preprocessed subject data files are stored
    :param exclude: optional. column names that should be ignored
    (not added to the subject)
    :param override: optional. if the subject already has a background item
    with the same name, should this be overriden by the new data?
    ...
    for subject in new_bgdata[linking_col]:
        if str(subject) in existing_subs:
            ...
            for column in group_colnames:
                if column not in temp_sub.has_background() or \
                    override is True:
                    temp_sub.add_background(column,
                                            new_bgdata
                                            .loc[new_bgdata[linking_col]
                                                 == subject]
                                            [column].values[0])
...
def combine_data(dataloc, subnums, groups=None,
                 save=False, noImages=False):
    """
    Combines a data set from subjects who have been written to file.

    :param dataloc: where the subject data files have been saved.
    Assumes .json files for subjects and stimuli are located in this folder
    :param subnums: which subjects to combine (list).
    Assumes one .json file per subject
    :param groups: list of group definitions to be included,
    must be same length and in same order as subnums
    :param save: do you want the combined data set saved into file? (Boolean)
    :param noImages: do you want to just combine background data?
    (useful for extremely large data sets)
    :return: combined data for the defined subjects.
    The data are stored in a dictionary,
    where each body map will be presented as N * X * Y numpy array,
    where N is length of subnums, and X and Y are the dimensions of the maps.
    The dictionary will have keys for each stimulus
    (with the value being 3-D Numpy array) and 'subids'
    (list of the subids in the same order as they appear
    in the data arrays).
    """

    # NB: replication of code for saving with/without images.
    # TODO: edit to remove repeat code

    filename = dataloc + '/dataset_' + \
        datetime.now().strftime("%d%m%Y-%H%M") + '.h5'
    ...
    if noImages:
        for j, subnum in tqdm(enumerate(subnums), desc="subjects"):
            ...
            if sum(all_res['bg']['subid'] == subnum) == 0:
                ...
                for bgkey, bgvalue in temp_sub.bginfo.items():
                    # cannot be neatly converted to numeric, excluding for now
        ...
        for key in tqdm(stim.all.keys(), desc="bodymap number"):
            ...
            if stim.all[key]['onesided']:
                data_matrix = np.zeros((len(subnums), size_onesided[0],
                                        size_onesided[1]))
            else:
                data_matrix = np.zeros((len(subnums), size_twosided[0],
                                        size_twosided[1]))
            for j, subnum in tqdm(enumerate(subnums), desc="subjects"):
                # print(subnum)
                temp_sub = Subject(subnum)
                ...
                if sum(all_res['bg']['subid'] == subnum) == 0:
                    ...
                    for bgkey, bgvalue in temp_sub.bginfo.items():
                        try:
                            # some variables cannot be neatly converted to
                            # numeric, excluding for now
                            float(bgvalue)
                            if not bgkey in all_res['bg'].columns:
                                all_res['bg'][bgkey] = np.nan
                            all_res['bg'].loc[subnum, bgkey] = int(bgvalue)
            ...
            if save:
                with h5py.File(filename, 'a') as store:
                    ...
                    if not have_written_bg:
                        ...
                        if groups is not None:
                            dt = h5py.special_dtype(vlen=str)
                            store.create_dataset('groups', data=groups,
                                                 dtype=dt)
                        for bgkey, bgvalue in all_res['bg'].items():
                            store.create_dataset(bgkey,
                                                 data=bgvalue.to_numpy(),
                                                 dtype="int32")
                        have_written_bg = True
...
def binarize(data, threshold=0.007):
    """
    Change data from colouring (with blur) binary(ish)
    coloured / not coloured format.
    If the data have only positive values,
    the returned map is truly binary (0/1).
    If the map also has negative values
    (e.g. emotion maps with activations and inactivations), the
    returned map can have three values: 0, 1, and -1.

    NB: Default threshold 0.007 chosen as limit based on
    what limit replicates coloring best in Aalto system (March 2019).
    The best value for this parameter will depend on brush & blur settings.
    If changed, you should definitely do a visual inspection
    of the result against known colouring.

    :param data: matrix with colouring data
    :return: same matrix, with coloured areas changed to
    1 and non-coloured changed to 0
    """
...
def count_pixels(data, mask=None, threshold=0.007):
    ...
    :param mask: optional. If provided, takes values inside mask into account
    in counting proportion colored
...
def compare_groups(group1, group2, testtype='t'):
    ...
    3-D data matrix of subject-wise colouring maps.
    Axis 0 represents subjects.
    :param group2: Data for group 2.
    3-D data matrix of subject-wise colouring maps.
    Axis 0 represents subjects.
    :param testtype: should the groups be compared using a
    two sample t-test (default) or z-test of proportions?
    :return: two matrices, with the test statistic
    and p-value for the comparison per each pixel
    """
    # copy the data for each group to
    # avoid accidentally making edits to original data
    ...
    if testtype == 'z':
        ...
        if np.amin(g0_data) < 0 or np.amin(g1_data) < 0:
            print('test of proportions is only defined for ' +
                  'data with no negative values')
            return
        # binarize and count hits
        g0_data = binarize(g0_data)
        g1_data = binarize(g1_data)
        successes = [np.concatenate(np.nansum(g0_data, axis=0)),
                     np.concatenate(np.nansum(g1_data, axis=0))]
        counts = [np.concatenate(np.nansum(~np.isnan(g0_data), axis=0)),
                  np.concatenate(np.nansum(~np.isnan(g1_data), axis=0))]
        # tried an ugly solution to division by 0
        # in cases of extreme difference
        successes[0] = successes[0]+1
        successes[1] = successes[1]+1

        # run proportions test for each pixel based on number of
        # observations(counts) and number of coloured pixels (successes)
        # this is a little slow, is there a better iteration?
        map_out = list(map(lambda x, y: proportions_ztest(x, y),
                           np.transpose(successes), np.transpose(counts)))
        statistics_twosamp = np.reshape(np.transpose(map_out)[0],
                                        (dims[1], dims[2]))
        pval_twosamp = np.reshape(np.transpose(map_out)[1],
                                  (dims[1], dims[2]))
    elif testtype == 't':
        # two sample t-test
        statistics_twosamp, pval_twosamp = stats.ttest_ind(g0_data,
                                                           g1_data,
                                                           axis=0,
                                                           nan_policy='omit')
    else:
        print("acceptable comparison types are 'z' and 't', " +
...
def correlate_maps(data, corr_with, method):
    """
    Correlates a set of subject-wise colouring maps with a vector of values
    (e.g. a background factor)

    :param data: 3-D data matrix of the subject-wise colouring maps.
    Axis 0 represents subjects.
    ...
    if method == 'spearmanr':
        ...
        raise ValueError(f"analysis type {method} is not implemented." +
...
def get_latest_datafile(datadir):
    ...
    for file in os.listdir(datadir):
        if file.startswith("dataset"):
            if latestfile == '' or os.path.getmtime(datadir + '/' + file) >\
                os.path.getmtime(datadir + '/' + latestfile):
                latestfile = file
            dataloc = os.path.join(datadir, latestfile)
    return dataloc


def make_qc_figures(subnums, indataloc, stimuli,
                    outdataloc=None):
    """
    Draw the entire colouring area for the given subjects.
    This is particularly useful in colouring quality control.
    ...
    for i, subnum in enumerate(subnums):
        print("making qc figures for subject " + str(subnum) + " which is " +
              str(i + 1) + "/" + str(len(subnums)))
        sub = Subject(subnum)
...
def make_correlation_plot(result_map_r, result_map_p, mask,
                          suptitle, outdataloc):
    ...
    masked_result_map_no_fdr = \
        np.ma.masked_where(mask != 1, result_map_r_no_fdr)
...
def read_in_mask(file1, file2=None):
    """
    Easily read in a black and white mask image and change to
    binary numpy array to use in other functions.
    If the data need left and right mask separately,
    please provide the mask to use on the left-hand side as the
    first argument.

    :param file1: Black-and-white mask image, where black shows
    areas inside the mask (i.e. to be included) and white
...
def p_adj_maps(pval_map, mask=None, alpha=0.05, method='fdr_bh'):
    """
    An easier interface to correct p-values for multiple comparisons.
    By default, implements False Detection Rate
    correction (Benjamini/Hochberg),
    but multiple other methods can be selected.

    Implements
    https://www.statsmodels.org/dev/generated/statsmodels.stats.multitest.multipletests.html

    :param pval_map: a 2-D matrix of p-values
    :param mask: optional. A boolean matrix with the areas inside the body
    outline set to 1/TRUE. If provided, will reduce the number of comparisons
    to correct for, as only areas inside the body outline
    (i.e. areas of interest) are considered.
    :param alpha: selected alpha-level (default 0.05)
    :param method:
    :return: 2-D matrix of the same size as first parameter, with p-values
    ...
    reject, pvals_corrected, alpacSidak, alhacBonferroni = \
        multipletests(data_reshaped, alpha, method)
    if mask is None:
...
def align_data(sublist, subarray, dataframe, subidcolumn='subid'):
    ...
    Sublist: a list of subject id's.
    This MUST be in the same order as the subject dimension in subarray
    Subarray: an array with embody data.
    Subject dimension MUST be the first dimension.
    Dataframe: a data frame with additional subject data
    to be aligned to the subarray.
    Subidcolumn: if the subject id column in the dataframe is not 'subid',
    ...
    common_subjects = [s for s in dataframe[subidcolumn].values if
                       s in subject_to_index]
    ...
    aligned_df = dataframe[dataframe[subidcolumn].isin(common_subjects)]\
        .copy().reset_index(drop=True)

    # Fill the aligned_data array with data from
    # data_array in the order of aligned_df
...
def count_pointserialr_for_array(data, corr_with):
    ...
    """
    # NB: The function is sensitive to missing values
    # which is why this is not implemented with
    # np.apply_along_axis
    dim1 = data.shape[1]
    dim2 = data.shape[2]
    ...
    for ind_i in range(0, dim1):
        for ind_j in range(0, dim2):
            ...
            problem_indices_bodymap = \
                np.argwhere(np.isnan(data[:, ind_i, ind_j]))
            all_problem_indices = \
                list(set(problem_indices_corrvariable.flatten()) |
                     set(problem_indices_bodymap.flatten()))

            # drop problematic values
            bodymap_bin_no_nans = np.delete(data[:, ind_i, ind_j],
                                            all_problem_indices)
            corr_variable_no_nans = np.delete(corr_with,
                                              all_problem_indices)

            if len(set(bodymap_bin_no_nans)) == 1 | \
                len(set(corr_variable_no_nans)) == 1:
                correlation = 0
                p_value = 1
            else:
                correlation, p_value =\
                    stats.pointbiserialr(bodymap_bin_no_nans,
                                         corr_variable_no_nans)
